...
current_status = [
    ...
    [0, 0, 0, 0],
    [0, 0, 0, 0],
]  # keep track of the safe spots in the map
...
def bfs(current, target):  #### used for going from one safe path to another                 #### using the current_status list which keeps track of all the safe paths
    visited = []
                ...
                for j1 in range(4):
                    if i1 == i and j1 == j:
                        ...
                        if (temp1 not in store) or (temp2 not in store):
                            ...
                            )  # there are atmost one wumpus
                            kb.append(
                                {(f"P{i+1}{j+1}", 0), (f"P{i1+1}{j1+1}", 0)}
                            )  # there are atmost one pit
...
def dpll(expr):
    ...
    if (
        len(expr_new) == 0
    ):  # early termination condition : if there are no clauses its true
        return True
    ...
    for a in expr:
        for b in a:
            ...
            if b[0] in holding_val:
                if (
                    b[1] != holding_val[b[0]]
                ):  # check if var already exists in the dict and mark it as false if other
...
def simulation(ag):
    ...
    current_status[0][
        0
    ] = 1  # current_status list which has places which are marked safe
